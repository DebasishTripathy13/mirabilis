# CoreStream: a streaming engine that lost, and what it measured

**This engine is not the recommended way to run anything.** It streams model
weights from RAM to the GPU on every token, and on a consumer laptop that is
about 4.5x slower than llama.cpp: 2.58 tok/s against 11.74 on comparable
models, measured on the same machine. The reason is in one line — PCIe moves
9.2 GiB/s while RAM reads at 36, so computing where the weights already sit
beats shipping them to the GPU.
